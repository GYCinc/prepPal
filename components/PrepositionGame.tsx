...
const optionVariants: Variants = {
  ...
  hover: { scale: 1.03, zIndex: 10, boxShadow: "0 0 25px rgba(239, 96, 53, 0.6)", borderColor: "#EF6035", transition: { duration: 0.1 } },
  tap: { scale: 0.95 },
};
...
const PrepositionGame: React.FC<PrepositionGameProps> = ({ 
  ...
  const generateQuestion = useCallback(async (index: number) => {
    ...
    try {
      const { correctPreposition: correctPrepositionItem } = generateRandomSentenceContext(level, category);

      const cachedQ = await findCachedQuestion(level, correctPrepositionItem.preposition, recentQuestionsRef.current);
                ...
                <div className="absolute top-4 left-4 right-4 flex justify-between items-start z-20 pointer-events-none">
                     <div className="flex gap-2 pointer-events-auto">
                         <div className="bg-black/60 backdrop-blur-md px-4 py-2 rounded-full border border-white/10 flex flex-col items-center leading-none shadow-lg">
                            <span className="text-[9px] uppercase tracking-widest text-gray-400 font-black">Round</span>
                     ...
                     <div className="flex gap-2 pointer-events-auto">
                        <div className="bg-black/60 backdrop-blur-md px-4 py-2 rounded-full border border-white/10 flex flex-col items-end leading-none shadow-lg">
                            <span className="text-[9px] uppercase tracking-widest text-gray-400 font-black">Score</span>
                            ...
                            >
                                 <div className="flex flex-col md:flex-row items-center justify-between w-full h-full p-6 md:p-8 relative gap-4">
                                             ...
                                             {selectedAnswer === currentQuestion.correctAnswer ? (
                                                 <>
                                                     <div className="bg-emerald-500/20 p-2 rounded-full border border-emerald-400/30 shadow-lg shadow-emerald-500/20 shrink-0">
                                                        <svg className="w-10 h-10 text-emerald-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="4" d="M5 13l4 4L19 7" /></svg>
                                                     </div>
                                                 ...
                                                 <>
                                                     <div className="bg-red-500/20 p-2 rounded-full border border-red-400/30 shadow-lg shadow-red-500/20 shrink-0">
                                                        <svg className="w-10 h-10 text-red-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="4" d="M6 18L18 6M6 6l12 12" /></svg>
                                                     </div>
                                     ...
                                     <div className="flex flex-row md:flex-col gap-4 relative z-10 md:min-w-[180px] justify-center w-full md:w-auto shrink-0">
                                         ...
                                         >
                                             Next
                                             <svg className="w-6 h-6 md:w-7 md:h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M13 7l5 5m0 0l-5 5m5-5H6"></path></svg>
                                         </button>
                                         {selectedAnswer === currentQuestion.correctAnswer ? (
                                              <button onClick={handleSpeakPreposition} className="flex-1 md:flex-none text-emerald-100 text-base md:text-lg font-bold hover:text-white flex items-center justify-center gap-2 py-3 md:py-4 bg-black/30 rounded-xl hover:bg-black/50 transition-all border border-white/10">
                                                 <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15.536 8.464a5 5 0 010 7.072m2.828-9.9a9 9 0 010 12.728M5.586 15H4a1 1 0 01-1-1v-4a1 1 0 011-1h1.586l4.707-4.707C10.923 3.663 12 4.109 12 5v14c0 .891-1.077 1.337-1.707.707L5.586 15z"></path></svg>
                                                 Pronounce
                                              </button>
                                         ) : (
                                             <button onClick={() => setShowCoreMeaning(!showCoreMeaning)} className="flex-1 md:flex-none text-red-100 text-base md:text-lg font-bold hover:text-white flex items-center justify-center gap-2 py-3 md:py-4 bg-black/30 rounded-xl hover:bg-black/50 transition-all border border-white/10">
                                 ...
                                 <AnimatePresence>
                                     {showCoreMeaning && selectedAnswer !== currentQuestion.correctAnswer && (
                                         ...
                                         >
                                             <div>
                                                <h4 className="text-gray-400 uppercase text-sm font-bold tracking-widest mb-3">Definition</h4>
                                                <p className="text-2xl md:text-3xl text-white font-medium leading-relaxed">
                                                    <span className="text-[#EF6035] font-black text-3xl md:text-4xl block mb-4">"{currentQuestion.correctAnswer}"</span>
                                                    {PREPOSITION_DETAILS[currentQuestion.correctAnswer]}
                                                </p>
                                             </div>
                                             <button onClick={() => setShowCoreMeaning(false)} className="absolute top-6 right-6 text-gray-400 hover:text-white bg-white/10 p-3 rounded-full hover:bg-white/20 transition-colors">
